...
def plot_x(attFile, benFile, idx):
    ...
    axs[0].plot(timestamp1, x1, label = 'Original'+str(idx))
    axs[0].plot(timestamp2, x2, label = 'Attacked'+str(idx))
    # axs[0].set_title('X-axis change')
    axs[0].set(xlabel='time(s)', ylabel='x(m)')
    axs[0].legend()
    axs[1].plot(timestamp1, y1, label = 'Original'+str(idx))
    axs[1].plot(timestamp2, y2, label = 'Attacked'+str(idx))
    # axs[1].set_title('Y-axis change')
    axs[1].set(xlabel='time(s)', ylabel='y(m)')
    axs[1].legend()
    # axs[2].plot(timestamp1, z1, label = 'Original'+str(idx))
    # axs[2].plot(timestamp2, z2, label = 'Attacked'+str(idx))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', dest='idx')
    args = parser.parse_args()

    plot_x(args.att_file, args.ben_file, args.idx)
