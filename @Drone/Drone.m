    ...
    methods
        ...
        function update_state(self, wind, time, i_drone)
            ...
            elseif self.drone_type == "point_mass"
                ...
                % Attack ----------
%                 self.vel_xyz = self.command(2:4);
%                 self.real_pos_ned = self.real_pos_ned + self.vel_xyz * self.p_sim.dt; % real gps pos
%                 spoof_pos = self.pos_ned + self.vel_xyz * self.p_sim.dt; % gps pos when attacked
%                 fileID = fopen('/media/EDrive/swarmlab_e/flag.txt', 'r'); % each digit is 0/1, indicating attacking or not for a certain drone
%                 flag = fscanf(fileID, '%f');
%                 fclose(fileID);
%                 idx = find(flag); %  parse file content
%                 
...
%                 end
%                 
%                 pos_time = [time, self.pos_ned.']; % record the position for each drone
